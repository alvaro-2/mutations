...
CREATE TABLE IF NOT EXISTS `mlo_mutations`.`protein_has_mlo` (
  ...
  UNIQUE INDEX `id_all_UNIQUE` (`id_protein` ASC, `id_dataset` ASC, `id_mlo` ASC, `reviewed` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mlo_mutations`.`mutation_has_llps` (
  ...
  CONSTRAINT `fk_llps_regions_has_mutation_llps_regions1`
    FOREIGN KEY (`id_llps`)
    REFERENCES `mlo_mutations`.`llps_regions` (`id_llps`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_llps_regions_has_mutation_mutation1`
    FOREIGN KEY (`id_mutation`)
    REFERENCES `mlo_mutations`.`mutation` (`id_mutation`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
