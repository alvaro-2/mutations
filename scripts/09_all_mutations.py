...
if __name__ == "__main__":
    ...
    # mutations clinvar    
    mutations = pd.read_csv(os.path.join(opts.inoutfolder, 'mutations_clinvar.tsv.gz'), sep='\t', compression='gzip')
    ...
    first_id_mut = max(mutations["id_mutation"].tolist()) + 1
    #mutation cosmic
    mutations_cosmic_all = pd.read_csv(os.path.join(opts.inoutfolder, 'mutations_parse_cosmic.tsv.gz'), sep='\t', compression='gzip')
    
    mutations_cosmic = mutations_cosmic_all[['id_protein', 'chromosome', 'start_genomic', 'end_genomic', 'notation_aa', 'notation_cds', 'start_aa', 'end_aa', 'consequence']].drop_duplicates()
